...
void queryDatabase(const  vector<cv::Mat> &features_query, myImage image[] ,vector<string> images_query) {
    ...
    Database db("/home/jin/Desktop/QueryResults/small_db.yml.gz");
...
void selfEvaluate(const  vector<cv::Mat> &features, myImage image[]) {
    ...
    int featuresSize = features.size() -5;
	...
	for(int j = 0; j < 5; j++)
	{
		...
		// cout <<"db reset" <<endl;
		vector<int> queryID;
		// cout <<"queryID reset" <<endl;

		for(size_t i = 0; i < featuresSize/5; i++)
		{  
			for (int k = 0; k<5;k++){
				if(k ==j){
					// cout<< "failed"<<endl;
					queryID.push_back(index);	
					// cout<< "got here"<<endl;
					// cout<<index<<endl;
				}
				else{
					db.add(features[index]);
				}
				index = index+1;
		...
		for(size_t i = 0; i < featuresSize/5; i++)
		{  
			db.query(features[queryID[i]], ret, 2);
	    	if (image[ret[1].Id].getLabel() == image[queryID[i]].getLabel()){
	    		resultN = resultN + 1;
	    		// cout << "Result rate: " << resultN <<endl;
	    	}
	    	else{
	    		// cout << "Mismatch!"<<endl;
	    	}
	    }
	    // cout << "resultN "<<resultN<<endl;
	}
	// cout << "features.size() " << features.size() <<endl;
	// cout << "resultN "<<resultN<<endl;
	float rate = (float) resultN / featuresSize;
    cout << endl << "Accuracy: " << rate <<endl <<endl;
    // cout << endl;
}
...
int main (int argc, char *argv[]) {
    ...
    for(int i = 0; i < train_dirNames.size(); i++){
        ...
        //imageDir = train_dirNames[i]+"/color";
