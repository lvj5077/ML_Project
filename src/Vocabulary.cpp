	...
	std::cout <<"HI f1"<<std::endl;
        ...
        if ( filename.find(".txt")!=std::string::npos){
            //read from a text file (used in ORBSLAM2)
						//std::cout <<"Hi fi2"<<std::endl; 
            load_fromtxt(filename);
        }
        else{
						//	std::cout <<"Hi f3 "<<std::endl; 
        cv::FileStorage fs(filename.c_str(), cv::FileStorage::READ);
						//std::cout <<"Hi f4 "<<std::endl;
        if(!fs.isOpened()) throw std::string("Could not open file ") + filename;
	//std::cout<<" Hi f5 "<< std::endl;
        this->load(fs);
        }
    }
}
  ...
  const std::string &name)
{
  ...
  // std::cout<<"I am here 6.5 fn.size = "<<fn.size()<<std::endl;
  for(unsigned int i = 0; i < fn.size(); ++i)
  {
    ...
    //std::cout<<"DescManip fromstring: nid = "<<nid<<" d = "<<d<<std::endl; 
